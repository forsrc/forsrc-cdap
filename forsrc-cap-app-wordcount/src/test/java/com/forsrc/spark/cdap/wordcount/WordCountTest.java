...
public class WordCountTest extends TestBase {
    ...
    private static final Type OBJECT_MAP_TYPE = new TypeToken<Map<String, Object>>() {
    }.getType();
    ...
    public void testWordCount() throws Exception {
        ...
        ApplicationManager appManager = deployApplication(WordCountApplication.class, config);

        // validate that the wordCount table is empty, and that it has no entry for
        // "world"
        DataSetManager<KeyValueTable> datasetManager = getDataset(config.getCounterTableName());
        KeyValueTable wordCounts = datasetManager.get();
        Assert.assertNull(wordCounts.read("world"));

        // Start the Flow
        FlowManager flowManager = appManager.getFlowManager("WordCounterFlow").start();
        ...
        // start a new transaction so that the "wordCounts" dataset sees changes made by
        // the flow
        datasetManager.flush();
        Assert.assertEquals(3L, Bytes.toLong(wordCounts.read("world")));

        // Start RetrieveCounts service
        ServiceManager serviceManager = appManager.getServiceManager(WordCountService.SERVICE_NAME).start();
